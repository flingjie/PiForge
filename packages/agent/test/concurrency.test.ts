...
import type { ToolSet } from "../src/graph/types.js";
...
describe("parallel", () => {
  ...
  it("runs nodes in parallel and waits for all", async () => {
    ...
    const results = await parallel(nodes, state, getTools);
    const elapsed = Date.now() - start;

    // All should complete in roughly the time of the slowest node.
